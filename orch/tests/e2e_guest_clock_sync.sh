#!/usr/bin/env bash
# Reproduce+verify: does the guest clock get synced on cold boot, unconditionally,
set -euo pipefail
...
RUN_ROOT="${GUEST_CLOCK_SYNC_RUN_ROOT:-$TRUSTED_RUN_BASE/guest-clock-sync}"
LOCK_FILE=/run/lock/taritd-guest-clock-sync.lock
RUN_DIR=
RUN_DIR_CREATED=0
ROOTFS=
SERVER_LOG=
...
export TARIT_API_KEY="guest-clock-sync-key"
export TARIT_LISTEN="127.0.0.1:8095"
...
export TARIT_BASE_URL="http://127.0.0.1:8095"
...
server_listener_is_current() {
  server_pid_is_current &&
    ss -lntp "sport = :8095" 2>/dev/null |
      grep -F "127.0.0.1:8095" |
...
guest_epoch() {
  local vm_id=$1 reply
  reply=$("$TARIT" --json exec "$vm_id" "date -u +%s") ||
    fail "could not read guest clock for $vm_id"
  printf '%s' "$reply" | python3 -c 'import json,sys; print(json.load(sys.stdin)["stdout"].strip())'
}

echo "--- checking guest clock right after cold boot (create_local, no external caller involved) ---"
HOST_EPOCH=$(date -u +%s)
GUEST_EPOCH=$(guest_epoch "$VM_A")
[ -n "$GUEST_EPOCH" ] && [ "$GUEST_EPOCH" -ge 0 ] 2>/dev/null ||
  fail "guest did not return a usable epoch timestamp: '$GUEST_EPOCH'"
DIFF=$((HOST_EPOCH - GUEST_EPOCH))
[ "$DIFF" -ge 0 ] || DIFF=$((-DIFF))
echo "host_epoch=$HOST_EPOCH guest_epoch=$GUEST_EPOCH diff=${DIFF}s"
[ "$DIFF" -lt 60 ] ||
  fail "guest clock is not synced after cold boot: off by ${DIFF}s (host=$HOST_EPOCH guest=$GUEST_EPOCH) - no RTC, and nothing set it"

echo "RESULT: GUEST_CLOCK_SYNC_PASS diff=${DIFF}s"

echo "--- checking guest clock after restart_policy sweep (taritd process restart, zero external callers) ---"
VM_B_JSON=$(curl -fsS -X POST -H "X-API-Key: $TARIT_API_KEY" -H 'Content-Type: application/json' \
  -d '{"vcpus":1,"memory_mib":256,"restart_policy":"always"}' \
  "$TARIT_BASE_URL/v1/vms")
VM_B=$(printf '%s' "$VM_B_JSON" | python3 -c 'import json,sys; print(json.load(sys.stdin)["id"])')
VM_IDS+=("$VM_B")
record_vm_process "$VM_B"
TAP_B=$(tap_for_vm "$VM_B")
record_vm_tap "$VM_B" "$TAP_B"

curl -fsS -X POST -H "X-API-Key: $TARIT_API_KEY" "$TARIT_BASE_URL/v1/vms/$VM_B/stop" >/dev/null
for _ in $(seq 1 20); do
  STATUS=$(curl -fsS -H "X-API-Key: $TARIT_API_KEY" "$TARIT_BASE_URL/v1/vms/$VM_B" |
    python3 -c 'import json,sys; print(json.load(sys.stdin).get("status","?"))')
  [ "$STATUS" = stopped ] && break
  sleep 1
done
[ "$STATUS" = stopped ] || fail "VM_B did not reach stopped before taritd restart (status=$STATUS)"

stop_taritd
start_taritd

for _ in $(seq 1 30); do
  STATUS=$(curl -fsS -H "X-API-Key: $TARIT_API_KEY" "$TARIT_BASE_URL/v1/vms/$VM_B" 2>/dev/null |
    python3 -c 'import json,sys; print(json.load(sys.stdin).get("status","?"))' 2>/dev/null)
  [ "$STATUS" = running ] && break
  sleep 1
done
[ "$STATUS" = running ] ||
  fail "restart_policy sweep did not bring VM_B back to running (status=${STATUS:-<none>})"

HOST_EPOCH_B=$(date -u +%s)
GUEST_EPOCH_B=$(guest_epoch "$VM_B")
[ -n "$GUEST_EPOCH_B" ] && [ "$GUEST_EPOCH_B" -ge 0 ] 2>/dev/null ||
  fail "guest B did not return a usable epoch timestamp: '$GUEST_EPOCH_B'"
DIFF_B=$((HOST_EPOCH_B - GUEST_EPOCH_B))
[ "$DIFF_B" -ge 0 ] || DIFF_B=$((-DIFF_B))
echo "host_epoch=$HOST_EPOCH_B guest_epoch=$GUEST_EPOCH_B diff=${DIFF_B}s"
[ "$DIFF_B" -lt 60 ] ||
  fail "guest clock is not synced after restart_policy sweep cold boot: off by ${DIFF_B}s - the exact original incident path"

echo "RESULT: GUEST_CLOCK_SYNC_SWEEP_PASS diff=${DIFF_B}s"
