#!/usr/bin/env bash
# Reproduce+verify: does a VM's egress allowlist survive a cold stop->start cycle
set -euo pipefail
...
RUN_ROOT="${EGRESS_COLDSTART_RUN_ROOT:-$TRUSTED_RUN_BASE/egress-coldstart}"
LOCK_FILE=/run/lock/taritd-egress-coldstart.lock
RUN_DIR=
RUN_DIR_CREATED=0
ROOTFS=
SERVER_LOG=
...
export TARIT_API_KEY="egress-coldstart-key"
export TARIT_LISTEN="127.0.0.1:8081"
...
export TARIT_BASE_URL="http://127.0.0.1:8081"
...
expect_guest_success "$VM_A" 'command -v sh curl ip'
...
expect_guest_denial "$VM_A" "$GUEST_CURL"
echo "OK: fresh VM defaults to deny-all egress"

curl -fsS -X PATCH \
  -H "X-API-Key: $TARIT_API_KEY" \
  -H 'Content-Type: application/json' \
  -d "{\"allowlist\":[\"$EGRESS_TEST_IP/32:$EGRESS_TEST_PORT/tcp\"],\"allow_existing\":true}" \
  "$TARIT_BASE_URL/v1/egress/vm/$VM_A" >/dev/null

assert_recovered_policy "$VM_A" "$TAP_A" "$GUEST_A" "$UPLINK"
expect_guest_success "$VM_A" "$GUEST_CURL"
echo "OK: egress allowlist enforced immediately after PATCH"

vm_status() {
  curl -fsS -H "X-API-Key: $TARIT_API_KEY" "$TARIT_BASE_URL/v1/vms/$1" |
    python3 -c 'import json,sys; print(json.load(sys.stdin)["status"])'
}

wait_for_vm_status() {
  local vm_id=$1 want=$2 got=
  for _ in $(seq 1 60); do
    got=$(vm_status "$vm_id") || true
    [ "$got" = "$want" ] && return
    sleep 1
  done
  fail "VM $vm_id did not reach status '$want' (last seen: '${got:-<none>}')"
}

echo "--- cold stop (frees the tap + slot, matching a real Huntaway pause) ---"
curl -fsS -X POST -H "X-API-Key: $TARIT_API_KEY" "$TARIT_BASE_URL/v1/vms/$VM_A/stop" >/dev/null
wait_for_vm_status "$VM_A" stopped

echo "--- cold start with NO external re-apply of egress (this is the bug under test) ---"
curl -fsS -X POST -H "X-API-Key: $TARIT_API_KEY" "$TARIT_BASE_URL/v1/vms/$VM_A/start" >/dev/null
wait_for_vm_status "$VM_A" running

TAP_A=$(tap_for_vm "$VM_A")
GUEST_A=$(guest_ip_for_tap "$TAP_A")

echo "--- verifying the egress allowlist survived the cold start on its own ---"
POST_COLDSTART_EGRESS_CHAIN=$(nft -a list chain ip taritd_nat vm_egress)
require_rule "$POST_COLDSTART_EGRESS_CHAIN" \
  "ip saddr $GUEST_A ct state established,related accept comment \"taritd-egress slot=${TAP_A#insta} vm=$VM_A tap=$TAP_A\"" ||
  fail "cold start lost the stateful-return egress rule for $VM_A (net.rs reset the tap to fresh-allocation defaults instead of reapplying the persisted allowlist)"
require_rule "$POST_COLDSTART_EGRESS_CHAIN" \
  "ip saddr $GUEST_A ip daddr $EGRESS_TEST_IP tcp dport $EGRESS_TEST_PORT accept comment \"taritd-egress slot=${TAP_A#insta} vm=$VM_A tap=$TAP_A\"" ||
  fail "cold start lost the PATCHed egress allowlist entry for $VM_A (net.rs reset the tap to fresh-allocation defaults instead of reapplying the persisted allowlist)"
assert_recovered_policy "$VM_A" "$TAP_A" "$GUEST_A" "$UPLINK"
expect_guest_success "$VM_A" "$GUEST_CURL"

echo "RESULT: EGRESS_COLDSTART_PASS"
